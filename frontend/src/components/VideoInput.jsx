...
export default function VideoInput({ onSetVideo, onAddToPlaylist, allowPlayNow = true }) {
  ...
  return (
    <form className="flex flex-col gap-2 px-4 py-3 bg-panel2 rounded-blob">
      ...
      <input
        ...
        className="bg-panel text-cream placeholder:text-mist rounded-full px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-banana"
      />
      <input
        value={title}
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          disabled={!url.trim()}
          className="flex-1 bg-panel text-cream font-display font-semibold rounded-full py-2.5 text-sm disabled:opacity-40 active:scale-95 transition-transform"
        >
          У черту
        </button>
      </div>
